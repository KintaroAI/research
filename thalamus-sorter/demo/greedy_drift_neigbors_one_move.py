...
class Sorter:
    ...
    def __init__(self, width, height):
        ...
        self.k = 99 # 24  # KNN count
        self.k_cache = {}
        #self.skip = {}
        self.image = self.load_image('./input.png')
    ...
    def init_coordinates(self):
        coordinates = np.zeros((self.neurons_count, 2), dtype=np.int32)
        for y in range(self.height):
            for x in range(self.width):
                i = self.neurons_matrix[y][x]
                coordinates[i][0] = x
    ...
    def get_neighbors(self, x, y):
        ...
        if (i, self.k) in self.k_cache:
            ...
            kj = np.argpartition(self.weight_matrix[i], -self.k)
            self.k_cache[(i, self.k)] = kj

        for j in kj[-self.k:-1]:
    ...
    def show(self):
        if False:
            ...
            #cv2.imwrite('perfect_weights.png', weight_matrix_normalized_frame)
