...
def save(gray1, gray2,  output_count):
    ...
    file_name = './output/output%06d.png' % output_count
    cv2.imwrite(file_name, joined_image)
    ...
    while True:
        ...
        if tick >= steps:
            ...
            for i, neighbors in enumerate(neighbors_indices):
                for i2, j in enumerate(neighbors):
                    if i != j:
                        corr = correlation(temporal_output, temporal_output, i, j)
                        #print(corr, distances[i][i2])
                        #update_weights(weight_matrix, abs(1.0-distances[i][i2]), get_neuron(W1, i), get_neuron(W1, j))
                        update_weights(weight_matrix, abs(corr), get_neuron(W1, i), get_neuron(W1, j))
        ...
        save(
            cv2.normalize(np.reshape(input, (CAMERA_DIM[1], CAMERA_DIM[0])), None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U),
            cv2.normalize(weight_matrix[:, :, 0], None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U),
            tick
        )
    cap.release()
