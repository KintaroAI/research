...
def get_gray_image(cap):
    ...
    return gray_vector, resized_frame

def show_color(color_vector):
    # Reshape the flattened grayscale frame to its original size
    color_frame = np.reshape(color_vector, (CAMERA_DIM[1], CAMERA_DIM[0], 3))
...
def tick():
    ...
    while True:
        step = tick % steps
        tick += 1
        temporal_input[:, step], original = get_gray_image(cap)
        ...
        neon = np.zeros(original.shape, dtype=np.uint8)
        variance = np.reshape(variance, (CAMERA_DIM[1], CAMERA_DIM[0]))
        neon_gray = cv2.normalize(
            variance, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
        neon[:, :, 0] = neon_gray[:]
        #neon_gray = np.reshape(neon_gray, (CAMERA_DIM[1], CAMERA_DIM[0]))
        _, neon_thresh = cv2.threshold(neon_gray, 10, 255, cv2.THRESH_BINARY)
        # Invert the thresholded neon image
        neon_thresh_inv = cv2.bitwise_not(neon_thresh)

        # Convert the inverted thresholded neon image to a color image
        neon_thresh_inv = cv2.cvtColor(neon_thresh_inv, cv2.COLOR_GRAY2BGR)

        # Merge the original and neon images using the thresholded neon image as a mask
        neon_thresh_color = cv2.cvtColor(neon_thresh, cv2.COLOR_GRAY2BGR)
        merged_image = cv2.bitwise_and(original, neon_thresh_inv) + cv2.bitwise_and(neon, neon_thresh_color)


        cv2.imshow("Frame", merged_image)
        cv2.waitKey(1)

        #show_color(merged_image)
        #print(max(variance))
        #cv2.waitKey(1)
    cap.release()
