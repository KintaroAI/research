...
def rebuild_output(sorted_neurons_matrix, output):
    ret = np.zeros(sorted_neurons_matrix.shape)
    for i, j in enumerate(sorted_neurons_matrix):
        ret[i] = output[j]
    return ret
...
def tick():
    ...
    weight_matrix_ideal = create_weight_matrix(output_dim)
    ...
    tick = 0
    neurons_matrix = np.arange(output_dim).reshape((CAMERA_DIM[1], CAMERA_DIM[0]))
    while True:
        step = tick % steps
        tick += 1
        ...
        if tick >= steps:
            ...
            distances, neighbors_indices = nn.kneighbors(return_distance=True)
            ...
            sorted_neurons_matrix = greedy_sort(neurons_matrix, weight_matrix[:, :,0])
            # Uncomment next line instead to see how ideal sorting is working.
            #sorted_neurons_matrix = greedy_sort(neurons_matrix, weight_matrix_ideal)
            show(rebuild_output(sorted_neurons_matrix, z1))
            #show(z1)
        cv2.waitKey(1)
        cv2.imshow(
            "Weights",
            cv2.normalize(weight_matrix[:, :, 0], None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
        )
    cap.release()
