#!/usr/bin/env python3
"""Compose a grid of experiment results into a single image.

Top-left cell: original image. Top row: K labels. Left column: MF labels.
...
CELL_SIZE = 80
LABEL_W = CELL_SIZE   # same as cell so original image fits at 80x80
LABEL_H = CELL_SIZE
PADDING = 2
...
n_rows = len(move_fractions)
n_cols = len(k_values)

# Total image size: label column + cells, label row + cells
img_w = LABEL_W + n_cols * (CELL_SIZE + PADDING) - PADDING
img_h = LABEL_H + n_rows * (CELL_SIZE + PADDING) - PADDING

# Load original image for top-left cell
...
for frame_idx in range(n_frames):
    ...
    # Top-left: original image at native 80x80
    canvas[0:CELL_SIZE, 0:CELL_SIZE] = original

    # Top row: K labels (centered in each cell)
    ...
    # Left column: MF labels (centered in each cell)
    for ri, mf in enumerate(move_fractions):
    ...
    out_path = os.path.join("output_grid", f"grid_{frame_idx:06d}.png")
