...
def create_column(column_type, column_config):
    ...
    if column_type == 'conscience':
        ...
        return TemporalPrototypeColumn(
            ...
            corr_window=column_config.get('corr_window', None),
            multi_scale=column_config.get('multi_scale', False),
            pred_lr=column_config.get('pred_lr', 0.01),
            surprise_alpha=column_config.get('surprise_alpha', 0.5),
            surprise_beta=column_config.get('surprise_beta', 0.99),
            decorrelation=column_config.get('decorrelation', True),
            decor_lr=column_config.get('decor_lr', 0.01),
            activation=column_config.get('activation', 'entmax15'),
            theta_learn_scale=column_config.get('theta_learn_scale', 0.0),
            reseed_after=column_config.get('reseed_after', 1000),
            wta_mode=column_config.get('wta_mode', 'none'),
            lateral_inputs=column_config.get('lateral_inputs', False),
            lateral_input_k=column_config.get('lateral_input_k', 4))
