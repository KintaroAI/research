/*
GPT-2 Transformer with Banded Sparsity on FC2 (MLP down-projection)
Experiment: Apply diagonal band mask to fcprojw weights.
...
// FC2 weights are (L, C, 4*C) - we apply band mask to fcprojw (down-projection)

int BAND_ENABLED = 1;  // Set to 0 to disable banding for comparison
int BANDWIDTH = 128;   // Tunable: each input connects to ~BANDWIDTH outputs

// Kernel to create banded mask for FC2 weights
...
__global__ void create_band_mask_kernel(float* mask, int OC, int IC, int bandwidth) {
...
typedef struct {
    ...
    float mean_loss;
    float* cpu_losses;
    // Banded sparsity mask for FC2 weights
    float* fc_mask;  // (L, C, 4*C) mask for fcprojw
} GPT2;
...
// Initialize banded sparsity mask for FC2 weights
void init_fc_mask(GPT2 *model) {
    int L = model->config.num_layers;
    int C = model->config.channels;
    int IC = 4 * C;  // input channels to FC2
    int OC = C;      // output channels from FC2
    size_t mask_size = L * OC * IC;
    ...
    for (int l = 0; l < L; l++) {
        float* layer_mask = model->fc_mask + l * OC * IC;
        create_band_mask_kernel<<<grid_size, block_size>>>(layer_mask, OC, IC, BANDWIDTH);
    }
    cudaCheck(cudaGetLastError());
    ...
    printf("| FC2 connections       | %d / %zu (per layer: %d / %d)              |\n", 
           h_count, mask_size, h_count / L, OC * IC);
    ...
    printf("| FC2 mask mode         | gradients + post-update only                       |\n");
}
...
void gpt2_backward(GPT2 *model) {
    ...
    for (int l = L-1; l >= 0; l--) {
        ...
        // FC2 backward: apply mask to fcprojw weight gradients
        float* l_fc_mask = (model->fc_mask != NULL && BAND_ENABLED) ? model->fc_mask + l * C * 4*C : NULL;
        matmul_backward_masked(dl_bt4c, dl_fcprojw, dl_fcprojb, dresidual, l_fch_gelu, l_fcprojw, l_fc_mask, B, T, 4*C, C);
        
        gelu_backward(dl_bt4c, l_fch, dl_bt4c, B*T*4*C);
        
        // FC1 backward: no mask (dense)
...
void gpt2_update(GPT2 *model, float learning_rate, float beta1, float beta2, float eps, float weight_decay, int t) {
    ...
    // Re-apply mask to FC2 weights after update (to ensure sparsity is maintained)
