...
void error_usage() {
    fprintf(stderr, "Usage: ./generate_banded [options]\n");
    fprintf(stderr, "Options:\n");
    fprintf(stderr, "  -e <string> model checkpoint path (default = checkpoint_banded.bin)\n");
...
int main(int argc, char *argv[]) {
    const char* model_path = "checkpoint_banded.bin";
    int num_tokens = 256;
    const char* prompt = "";
    ...
    printf("=== GPT-2 Inference (Banded Sparsity Checkpoint) ===\n");
    printf("Model: %s\n", model_path);
    printf("Tokens: %d\n", num_tokens);
    printf("Seed: %llu\n", rng_state);
