...
With optional banded sparsity on FC1 and/or FC2 layers.

Usage:
  ./train                           # Dense (no sparsity, original behavior)
  ./train -1 256 -2 256             # Both FC1 and FC2 with bandwidth 256
  ./train -1 0 -2 128               # Only FC2 with bandwidth 128 (FC1 dense)
  ./train -1 256 -2 0               # Only FC1 with bandwidth 256 (FC2 dense)
...
int EMBED_PULL_WINDOW = 0;      // Hebbian pull window, 0 = disabled
float EMBED_PULL_EPS = 1e-5f;   // Hebbian pull strength
...
void embed_pull(float* wte, const int* inputs, float eps, int B, int T, int C, int W) {
    int N = B * T * C;
    int block_size = 256;
    int grid_size = CEIL_DIV(N, block_size);
    embed_pull_kernel<<<grid_size, block_size>>>(wte, inputs, eps, B, T, C, W);
...
// Masked version of matmul_backward for weight gradients (banded sparsity)
...
// ----------------------------------------------------------------------------
// Banded sparsity mask initialization
...
void error_usage() {
    fprintf(stderr, "Usage:   ./train [options]\n");
    ...
    fprintf(stderr, "  -H <int>    Hebbian pull window size (default = 0, disabled)\n");
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i+=2) {
        ...
        else if (argv[i][1] == 'H') { EMBED_PULL_WINDOW = atoi(argv[i+1]); }
    ...
    printf("| Hebbian pull window   | %-50d |\n", EMBED_PULL_WINDOW);
