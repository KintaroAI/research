"""
Create a small GPT model from scratch for training with llm.c CUDA.
Experiment 00005: From-scratch training on GPU
"""
import struct
import torch
...
class GPTConfig:
    ...
    n_layer: int = 4           # small: 4 layers
    n_head: int = 4            # 4 attention heads
    n_embd: int = 128          # 128 dim embedding
...
def write_model(model, filename):
    ...
    with open(filename, 'wb') as f:
        ...
        # Write parameters in llm.c order (all layers concatenated per param type):
        ...
        # 3. ln1w (L * C) - concatenate all layer norm 1 weights
        ...
        # 5. qkvw (L * 3C * C) - attention c_attn weights
    ...
    expected_size = 256 * 4 + expected_params * 4  # header + params
...
if __name__ == "__main__":
    ...
    # Save model
    write_model(model, "small_gpt_scratch.bin")
